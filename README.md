...
git clone https://github.com/tomaszwi66/write-transformer.git
...
<pre> 🔬 DEMO: Training the model Tokens: 1,847 Examples: 1,783 Batches: 222 ───────────────────────────────────────────── Epoch 1/15 │ Loss: 5.2341 │ PPL: 188.0 │ [░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░] │ 2s Epoch 3/15 │ Loss: 4.1023 │ PPL: 60.5 │ [█████░░░░░░░░░░░░░░░░░░░░░░░░░] │ 5s Epoch 6/15 │ Loss: 3.2156 │ PPL: 24.9 │ [████████████░░░░░░░░░░░░░░░░░░] │ 9s Epoch 15/15 │ Loss: 2.1847 │ PPL: 8.9 │ [██████████████████████████████] │ 21s ───────────────────────────────────────────── ✅ Done in 21.3s! Loss: 2.1847 </pre>
Sample output from Step 10 (Generation):
<pre> 🌡️ Temperature: Low (0.3) 'Kot siedzi' → Kot siedzi na macie i obserwuje ptaki za oknem 'Stary człowiek' → Stary człowiek czyta książkę w cichej bibliotece 🌡️ Temperature: High (1.5) 'Kot siedzi' → Kot siedzi radośnie kominku piękne warzywa 'Stary człowiek' → Stary człowiek łagodnie kwitną gwiazdy melodie </pre>
🚀 Direct Modes
...
/quit	Exit
🏗️ Architecture
This implements exactly the same architecture as GPT-2/3/4.
The only difference is scale.

<pre> write-transformer GPT-2 Small GPT-4 ──────────────────── ──────────────── ──────────────── ~512 token vocab 50,257 tokens ~100K tokens 64 dimensions 768 dimensions ~12,288 dimensions 4 heads 12 heads ~96 heads 4 layers 12 layers ~120 layers ~50K parameters 124M parameters ~1.8T parameters 25 sentences 8M web pages the entire internet seconds on CPU hours on 8 GPUs months on 25K GPUs </pre><pre> Token IDs [batch, seq_len] ↓ ┌──────────────────────────────┐ │ Token Embedding │ ID → learned vector │ + Positional Embedding │ position → learned vector │ + Dropout │ └──────────────┬───────────────┘ ↓ ┌──────────────────────────────┐ │ Transformer Block ×N │ ← repeat N times │ ┌────────────────────────┐ │ │ │ LayerNorm │ │ │ │ Multi-Head Attention │ │ Q, K, V + causal mask │ │ + Residual Connection │ │ x + attention(x) │ ├────────────────────────┤ │ │ │ LayerNorm │ │ │ │ Feed-Forward (GELU) │ │ d_model → 4×d_model → d_model │ │ + Residual Connection │ │ x + ffn(x) │ └────────────────────────┘ │ └──────────────┬───────────────┘ ↓ ┌──────────────────────────────┐ │ LayerNorm │ │ Linear → logits │ d_model → vocab_size │ (weight tying with embed) │ same matrix encodes & decodes └──────────────────────────────┘ ↓ Probabilities for next token </pre>
...
📁 Project Structure
<pre> write-transformer/ ├── write_transformer.py ← everything (single file, by design) ├── README.md ├── README_PL.md ← Polish version ├── LICENSE ├── requirements.txt └── .gitignore </pre>
...
<pre> checkpoint/ ├── model.pt ← trained weights ├── tokenizer.json ← BPE vocabulary & merges └── config.json ← architecture configuration </pre>
❓ FAQ
...
Make your changes
Verify python write_transformer.py works in all modes
...
<div align="center">
If this helped you understand Transformers - leave a ⭐

You don't need to understand 1.8 trillion parameters.
Understand 50 thousand - the rest is the same architecture, just bigger.
